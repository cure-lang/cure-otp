...
data CRun : Config -> Config -> Type where
  CRDone : CRun c c
  CRStep : (p : Role) -> (q : Role) -> (t : Tag) -> (k : Global) -> CRun (step_config (GMsg p q t k) p q) c3 -> CRun (config (GMsg p q t k)) c3

config_terminates : (g : Global) -> CRun (config g) (config GEnd)
config_terminates GEnd = CRDone
config_terminates (GMsg p q t k) = CRStep p q t k (rewrite config_fidelity p q t k in config_terminates k)
