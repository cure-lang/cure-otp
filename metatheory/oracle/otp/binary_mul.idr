...
-- VERIFIED BINARY MULTIPLICATION (on the ripple-carry adder). Naturals represented as bit lists (LSB first); toNat denotes them. Proved: the
...
doubleIsPlus : (x : Natt) -> double x = plus x x
doubleIsPlus Z = Refl
doubleIsPlus (S k) = snatCong (S (double k)) (plus k (S k)) (trans (snatCong (double k) (plus k k) (doubleIsPlus k)) (sym (plusSuccR k k)))

doubleTimes : (m : Natt) -> (n : Natt) -> double (times m n) = times (double m) n
doubleTimes m n = trans (doubleIsPlus (times m n)) (trans (sym (timesPlusL m m n)) (timesCongL (plus m m) (double m) n (sym (doubleIsPlus m))))

bmul : Bin -> Bin -> Bin
bmul BNil b = BNil
bmul (BCons O a2) b = BCons O (bmul a2 b)
bmul (BCons I a2) b = add b (BCons O (bmul a2 b))

mulCorrect : (a : Bin) -> (b : Bin) -> toNat (bmul a b) = times (toNat a) (toNat b)
mulCorrect BNil b = Refl
mulCorrect (BCons O a2) b = trans (doubleCong (toNat (bmul a2 b)) (times (toNat a2) (toNat b)) (mulCorrect a2 b)) (doubleTimes (toNat a2) (toNat b))
mulCorrect (BCons I a2) b = trans (addCorrect b (BCons O (bmul a2 b))) (plusCongR (toNat b) (double (toNat (bmul a2 b))) (times (double (toNat a2)) (toNat b)) (trans (doubleCong (toNat (bmul a2 b)) (times (toNat a2) (toNat b)) (mulCorrect a2 b)) (doubleTimes (toNat a2) (toNat b))))
